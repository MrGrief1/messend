...
function openWhiteboard() {
    if (!currentRoomId) {
        alert('Сначала выберите чат, чтобы открыть общую доску.');
        return;
    }

    const targetUrl = `/whiteboard?room_id=${encodeURIComponent(currentRoomId)}`;
    ...
    const whiteboardWindow = window.open(targetUrl, '_blank', 'noopener=yes');
    if (!whiteboardWindow) {
        window.location.href = targetUrl;
    }
}
