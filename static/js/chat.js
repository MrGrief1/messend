...
function openWhiteboard() {
    if (!currentRoomId) {
        alert('Выберите чат, чтобы открыть доску.');
        return;
    }

    const boardPath = `/whiteboard/${currentRoomId}`;
    const width = Math.min(window.screen?.width || 1280, 1400);
    const height = Math.min(window.screen?.height || 900, 900);
    const features = `noopener=yes,resizable=yes,scrollbars=yes,width=${width},height=${height}`;
    const boardWindow = window.open(boardPath, '_blank', features);

    if (!boardWindow) {
        const shareLink = `${window.location.origin}${boardPath}`;
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(shareLink)
                .then(() => alert('Ссылка на доску скопирована. Разрешите всплывающие окна для мгновенного открытия.'))
                .catch(() => alert(`Откройте доску вручную: ${shareLink}`));
        } else {
            alert(`Откройте доску вручную: ${shareLink}`);
        }
    }
}
